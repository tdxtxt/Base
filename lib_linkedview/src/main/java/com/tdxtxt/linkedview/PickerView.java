...
/**
 * Picker View
 * https://github.com/wzbos/Android-Linked-View
 */
public class PickerView extends RecyclerView {
    ...
    public void restore() {
        ...
        for (IPickerData d : data) {
            d.setCheckedItem(defaultIds != null && defaultIds.contains(d.getObjId()));
        }
    ...
    class PickerViewHolder extends ViewHolder implements OnClickListener {
        ...
        void bind(IPickerData bean) {
            ...
            int count;
            if (isShowPickCount && bean.isCheckedItem() && (count = getNextCount()) > 0) {
                tvName.setText(bean.getDisplayName() + "(" + count + ")");
            } else {
                tvName.setText(bean.getDisplayName());
            }
        }
