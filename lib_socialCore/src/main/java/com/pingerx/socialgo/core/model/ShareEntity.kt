...
class ShareEntity(val shareObjType: Int) : Parcelable {
    ...
    fun init(title: String?, summary: String?, thumbImagePath: String?, targetUrl: String?) {
        ...
        val image = thumbImagePath?: ShareEntity.THUMB_URL
        if(image.startsWith("http")) setThumbImagePathNet(image) else setThumbImagePath(image)
    ...
    fun getThumbImagePathNet(): String {
        return thumbImagePathNet + ""
    }
    ...
    fun setThumbImagePathNet(thumbImagePathNet: String?) {
        this.thumbImagePathNet = thumbImagePathNet
    }
    ...
    companion object CREATOR : Parcelable.Creator<ShareEntity> {
        ...
        fun buildImageObj(path: String?): ShareEntity {
            ...
            val image = path?: THUMB_URL
            if(image.startsWith("http")) shareMediaObj.setThumbImagePathNet(image) else shareMediaObj.setThumbImagePath(image)
            return shareMediaObj
        }
        ...
        fun buildMusicObj(title: String?, summary: String?, thumbImagePath: String?, targetUrl: String?, mediaPath: String?, duration: Int?): ShareEntity {
            ...
            shareMediaObj.duration = duration?: 0
            return shareMediaObj
        }
