...
public class DynamicWatermarkView extends View {
    ...
    private final int                watermarkSpeed      = LiteavPlayerUtils.getRandomNumber(3, 5);//速度
     ...
     * 设置数据
     */
    public void setData(String dynamicWatermarkTip, int tipTextSize, int tipTextColor) {
        if (!TextUtils.isEmpty(dynamicWatermarkTip)) {
            if (this.dynamicWaterConfig == null) {
                watermarkDegree = LiteavPlayerUtils.getRandomNumber(25, 65);
    ...
    private void calculateBgRectWH() {
        ...
        textPaint.setTextAlign(Paint.Align.CENTER);
        textPaint.setTextSize(dynamicWaterConfig.tipTextSize);
        int bgRectPadding = sp2px(4);       //文本的左右内边距之和 或 上下内边距之和
    ...
    static class DynamicWaterConfig implements Serializable {
        /**
         * 动态水印文本
         */
        ...
        DynamicWaterConfig(String dynamicWatermarkTip, int tipTextSize, int tipTextColor) {
            this.dynamicWatermarkTip = dynamicWatermarkTip;
            this.tipTextSize = dp2px(tipTextSize);
