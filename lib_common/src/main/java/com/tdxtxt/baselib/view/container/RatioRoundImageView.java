 ...
 * @since 2021/4/13
 */
public class RatioRoundImageView extends AppCompatImageView implements RoundMethodInterface, RatioMeasureDelegate {
    private RoundHelper mHelper = new RoundHelper();
    private RatioLayoutDelegate mRatioLayoutDelegate;

    public RatioRoundImageView(@NonNull Context context) {
        super(context);
        mHelper.init(context, null, this);
    }

    public RatioRoundImageView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        mRatioLayoutDelegate = RatioLayoutDelegate.obtain(this, attrs);
        mHelper.init(context, attrs, this);
    }

    public RatioRoundImageView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
