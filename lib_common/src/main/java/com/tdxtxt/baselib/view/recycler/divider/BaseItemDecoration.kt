...
abstract class BaseItemDecoration constructor(build: Builder) : RecyclerView.ItemDecoration() {
    ...
    override fun onDraw(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        for (index in 0 until childCount) {
            ...
            if((parent.layoutManager !is GridLayoutManager) && childPosition >= totalCountItem - 1){
                //隐藏分割线 网格不支持隐藏分割线
                continue
            }
    ...
    abstract class Builder(private val mContext: Context, layoutOrientation: Int) {
        ...
        internal var dividerWidth: Int = 2
        ...
        fun setDividerMargin(
            ...
            return setDividerMarginPx(
                SizeUtils.dp2px(leftMargin),
                SizeUtils.dp2px(topMargin),
                SizeUtils.dp2px(rightMargin),
                SizeUtils.dp2px(bottomMargin)
            )
        }

        fun setDividerMarginHorizontal(horizontalMargin: Float) : Builder {
            val px = SizeUtils.dp2px(horizontalMargin)
            return setDividerMarginPx(px, 0, px, 0)
        }

        fun setDividerMarginVertical(verticalMargin: Float) : Builder {
            val px = SizeUtils.dp2px(verticalMargin)
            return setDividerMarginPx(0, px, 0, px)
        }
        ...
        fun setDividerDrawable(drawable: Drawable): Builder {
            this.dividerDrawableProvider = object : DividerDrawableProvider{
                override fun getDividerDraw(position: Int, parent: RecyclerView): Drawable {
                    return drawable
                }
            }
            return this
        ...
        fun setDividerColor(color: Int): Builder {
            this.dividerColorProvider = object : DividerColorProvider{
