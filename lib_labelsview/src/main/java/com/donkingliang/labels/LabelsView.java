...
//            测试的数据
    ...
    private static final String KEY_USE_DEFAULT_SELECT = "key_use_default_select";
...
//        }
        ...
        bundle.putBoolean(KEY_USE_DEFAULT_SELECT, useDefaultSelect);
...
//            }
            ...
            useDefaultSelect = bundle.getBoolean(KEY_USE_DEFAULT_SELECT, useDefaultSelect);
            //恢复标签方向
...
//            }
    ...
    public void setLabels(List<String> labels, boolean useDefaultSelect) {
        setLabels(labels, useDefaultSelect, new LabelTextProvider<String>() {
            @Override
    ...
    public <T> void setLabels(List<T> labels, LabelTextProvider<T> provider){
        setLabels(labels, true, provider);
    }
     ...
     * @param <T>
     */
    public <T> void setLabels(List<T> labels, boolean useDefaultSelect, LabelTextProvider<T> provider) {
        //清空原有的标签
        innerClearAllSelect();
        removeAllViews();
        mLabels.clear();
        ...
        this.useDefaultSelect = useDefaultSelect;
        if (useDefaultSelect && mSelectType == SelectType.SINGLE_IRREVOCABLY) {
            setSelects(0);
        }
    }
    ...
    public <T> T getSelectLabelData(int position){
        List<T> list = getSelectLabelDatas();
        if(list == null) return null;
        if(list.size() > position) return list.get(position);
        return null;
    }
